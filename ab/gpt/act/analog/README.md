...
The analog experiments below intentionally use `ab.gpt.act.analog.TuneNNGenAnalog` and prompt config names under `analog/` so the paper-specific runs do not override those baseline entrypoints.
...
run_one_arm() {
  ...
  ${VENV}/bin/python -m ab.gpt.act.analog.TuneNNGenAnalog \
    --llm_conf "$llm_conf" \
...
| `python -m ab.gpt.act.analog.TuneNNGenAnalog --help` | pass |
| `python -m ab.gpt.act.analog.TuneNNGen_7B_code_olympic_analogical_smoke --help` | pass |
