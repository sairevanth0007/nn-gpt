...
class BBBLinear(ModuleWrapper):
    def __init__(self, in_features, out_features, bias=True, priors=None):
        ...
        if priors is None:
            self.prior_mu = 0
            self.prior_sigma = random.uniform(0.01, 0.2)
            self.posterior_mu_initial = (0, 0.1)
            self.posterior_rho_initial = (-5, random.uniform(0, 1))
        else:
...
def supported_hyperparameters():
    return {'lr', 'momentum', 'random_sigma', 'random_rho_initial', 'random_beta'}
...
class Net(ModuleWrapper):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.priors = {
            'prior_mu': 0,
            'prior_sigma': random.uniform(0.01, 0.2),
            'posterior_mu_initial': (0, 0.1),
            'posterior_rho_initial': (-5, random.uniform(0, 1)),
        }
